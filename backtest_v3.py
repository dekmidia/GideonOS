"""
╔══════════════════════════════════════════════════════════════════╗
║     CRYPTO PUMP BACKTEST v3 — Marcelo Vega                     ║
║     Modo Swing (TP 5%, Timeout 24h)                            ║
║     Proteção Anti-Bull Trap: Volume Triplo, RSI < 65, Stoch <80║
╚══════════════════════════════════════════════════════════════════╝
"""
...
# PERFIL SCALPING (Tese a ser comprovada)
PERFIL = {"tp": 1.02, "sl": 0.985, "expiry": 20}  # TP +2%, SL -1.5%, Timeout 5h
...
def detectar_v3(janela_df, hora_utc):
    ...
    macd_l, macd_sig, _ = macd_ind(close)
    ...
    # Squeeze deve ter sido ON e agora está OFF
    if not ((not sq_on.iloc[i]) and sq_on.iloc[i-1]): return None

    # TETO DE RSI (Obrigatório / Veto) - anti-esticamento
    if rsi_s.iloc[i] > 65: return None

    # ESTOCÁSTICO (Obrigatório / Veto) - anti-exaustão
    # %K deve estar cruzado acima de %D (bullish) mas abaixo da zona de sobrecompra (80)
    stoch_k, stoch_d = pctk.iloc[i], pctd.iloc[i]
    if not ((stoch_k > stoch_d) and (stoch_k < 80)): return None

    # FILTRO INSTITUCIONAL (VOLUME TRIPLO)
    # A vela de rompimento deve ter volume 3x maior que a soma das últimas 3 velas (dinheiro real)
    # OU 2.0x a média de 20 (explosão atípica)
    vol_atual = janela_df["volume"].iloc[i]
    vol_3_ant = janela_df["volume"].iloc[i-3:i].sum()
    volume_institucional = (vol_atual >= vol_3_ant) or (vol_atual >= 2.0 * vol_ma.iloc[i])
    if not volume_institucional: return None
    ...
    c_base = {
        ...
        "R": rsi_s.iloc[i] > 35, # teto já checado no veto
        "M": mfi_s.iloc[i] > 48,
    ...
    return {"preco": close.iloc[i] * 1.003, "score": score, "rsi": rsi_s.iloc[i], "k": stoch_k}
...
def backtest_v3():
    print("\n╔═════════════════════════════════════════════════════╗")
    print("║   BACKTEST v3 — FILTROS ANTI BULL-TRAP              ║")
    print("║   (RSI<65, Stoch<80 e Volume Triplo)               ║")
    ...
    for symbol in PARES_BACKTEST:
        ...
        for i in range(JANELA, len(df) - PERFIL["expiry"] - 1):
            if cooldown > 0:
                cooldown -= 1
                continue
            sinal = detectar_v3(df.iloc[i - JANELA: i], df.index[i].hour)
            if not sinal: continue
            
            df_swing = df.iloc[i: i + PERFIL["expiry"] + 1]
            ...
            trades_log.append({
                ...
                "rs": rs, "ret": ret, "rsi": sinal["rsi"], "k": sinal["k"]
            })
            trades += 1
            cooldown = 10
    ...
    print(f"\n{'═'*70}")
    print(f"  📊 RESULTADO v3 (Filtro Antiaramadilha)")
    print(f"{'═'*70}")
    
    if sinais_totais == 0:
        print("Nenhum sinal encontrado com as novas regras restritivas.")
        return
    ...
    if not trades_df.empty:
        print("  Detalhe dos sinais v3:")
        for r in trades_df.itertuples():
            ic = "✅" if r.rs=="WIN" else ("❌" if r.rs=="LOSS" else "⏳")
            print(f"  {r.symbol:<11} {r.data} {ic}{r.rs:<5} {r.ret:>+6.2f}% | RSI:{r.rsi:>4.1f} K:{r.k:>4.1f}")
